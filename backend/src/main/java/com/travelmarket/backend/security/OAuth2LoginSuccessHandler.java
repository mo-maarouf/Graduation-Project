...
public class OAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(
        ...
        String providerUserId = asString(oauthUser.getAttribute("sub"));   // stable Google user id
        String email = asString(oauthUser.getAttribute("email"));         // email
        String name = asString(oauthUser.getAttribute("name"));           // display name (optional)

        if (providerUserId == null || email == null) {
            // Missing required OAuth info; fail fast.
        ...
        if (!"Traveler".equals(roleCookie) && !"Guide".equals(roleCookie)) {
            response.sendError(400, "Missing role selection. Restart OAuth from role selector.");
            return;
        }
        ...
        if (existingIdentity.isPresent()) {
            ...
            // Optional safety: if the stored user role conflicts with roleCookie, do not change role.
            // We simply keep existing role and let frontend handle onboarding normally.
            ...
            if (user != null) {
                // Example: email is already Traveler, but user tried OAuth as Guide
            ...
            if (user == null) {
                ...
                // OAuth users don't need a real password now; generate a random hash.
                // Later, if you implement "set password", they can convert to email/password.
                user.setPasswordHash(passwordEncoder.encode(UUID.randomUUID().toString()));

                user.setRole(User.Role.valueOf(roleCookie));
                user.setAccountStatus("ACTIVE");

                // Keep agreements + profile completion false until user finishes onboarding.
                user.setAgreedToTerms(false);
                user.setAgreedToPrivacy(false);
                user.setAgreementsAcceptedAtUtc(null);
                user.setProfileCompleted(false);

                // If you want, prefill full name from Google, but user can edit it later.
                ...
                // Create role-specific profile row (this matches your DB schema and avoids null profile IDs).
            ...
            try {
                ...
                // If identity link already exists due to race or repeated callback, ignore safely.
                // The unique constraint will protect you from duplicates.
            }
        }

        // Issue your normal JWT (same as email/password flow)
        UserDetails userDetails = userDetailsService.loadUserByUsername(user.getEmail());
        String jwt = jwtUtil.generateToken(userDetails);

        // Clear role cookie (one-time use)
        clearCookie(response, "oauth_role");

        // Redirect to frontend callback with JWT and basic flags
