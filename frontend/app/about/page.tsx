...
const StatCard = ({ stat }: { stat: typeof STATISTICS[0] }) => {
  ...
  return (
    <div className="p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl hover:shadow-lg transition-all hover:-translate-y-1">
      <div className="inline-flex p-3 rounded-xl bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 mb-4">
...
const ValueCard = ({ value }: { value: typeof VALUES[0] }) => {
  ...
  return (
    <div className={`p-6 rounded-xl border-2 ${colorClasses[value.color as keyof typeof colorClasses]} hover:shadow-lg transition-all hover:-translate-y-1`}>
      <div className="inline-flex p-3 rounded-lg bg-white dark:bg-gray-900 mb-4">
        <Icon className="w-6 h-6" />
      </div>
              ...
              {TIMELINE.map((item, index) => (
                <div key={index} className="relative flex gap-6">
                  {index < TIMELINE.length - 1 && (
                    <div className="absolute left-5 top-12 bottom-0 w-0.5 bg-gray-300 dark:bg-gray-700" />
                  )}
                  
                  <div className="relative z-10">
                    <div className="w-10 h-10 rounded-full bg-blue-600 dark:bg-blue-500 text-white font-bold flex items-center justify-center">
                      {index + 1}
                    </div>
                  </div>

                  <div className="flex-1 pb-8">
                    <div className="inline-block px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-sm font-medium rounded-full mb-2">
                      {item.year}
                    </div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                      {item.title}
          ...
          <div className="container-safe mx-auto max-w-4xl text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 px-8 py-4 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white font-semibold rounded-xl transition-all"
              >
                Become a Guide
              </Link>
            </div>
